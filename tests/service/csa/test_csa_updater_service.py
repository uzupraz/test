...
class TestCsaUpdaterService(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.repository_mock = MagicMock(spec=CsaUpdaterRepository)
    ...
    def test_get_target_list_success(self):
        ...
        self.repository_mock.get_csa_machines_info.return_value = [machine_info]
        ...
        self.repository_mock.get_csa_module_versions.return_value = [module_info]

        self.s3_service_mock.generate_download_pre_signed_url.return_value = "https://fakeurl.com"

        response = self.csa_updater_service.get_target_list(
            self.owner_id, 
            self.machine_id, 
        ...
        self.assertEqual(len(response.target_list), 1)
        target_item = response.target_list[0]
        ...
        self.repository_mock.update_modules.assert_called_once()
        self.repository_mock.get_csa_machines_info.assert_called_once_with(owner_id=self.owner_id, machine_id=self.machine_id)
        self.repository_mock.get_csa_module_versions.assert_called_once_with(self.module_name)


    @patch.object(CsaUpdaterService, '_get_next_version')
    def test_get_target_list_no_update_required(self, mock_get_next_version):
        ...
        self.assertIsInstance(response, UpdateResponse)
        self.assertEqual(len(response.target_list), 1)
        self.assertEqual(response.target_list[0].version, current_version)


    def test_get_target_list_no_machine_info(self):
        self.repository_mock.get_csa_machines_info.return_value = []

        with self.assertRaises(ServiceException) as context:
            self.csa_updater_service.get_target_list(self.owner_id, self.machine_id, [])
    ...
    @patch.object(CsaUpdaterService, '_get_next_version')
    def test_get_target_list_multiple_modules(self, mock_get_next_version):
        ...
        self.repository_mock.get_csa_module_versions.side_effect = mock_get_module_versions
        ...
        self.assertIsInstance(response, UpdateResponse)
        self.assertEqual(len(response.target_list), 3)
        self.assertEqual(response.target_list[0].version, module1["next"])
        self.assertEqual(response.target_list[1].version, module2["next"])
        self.assertEqual(response.target_list[2].version, module3["next"])  
        ...
        self.repository_mock.update_modules.assert_called_once_with(
    ...
    def test_get_target_list_no_machine_info(self):
        ...
        with self.assertRaises(ServiceException) as context:
            self.csa_updater_service.get_target_list(
    ...
    def test_create_target_list_item(self):
            self.s3_service_mock.generate_download_pre_signed_url.return_value = "https://fakeurl.com"
            
            next_version = version.parse("1.0.1")
            module_version_info = [ModuleInfo(module_name=self.module_name, version="1.0.1", checksum="abc123")]
            
            target_item = self.csa_updater_service._create_target_list_item(
